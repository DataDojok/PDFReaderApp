...
async function loadPdfJs() {
  ...
  // PDF.js 6 requires a worker source even when it falls back to its
  // same-thread worker. Importing the worker module here lets Metro bundle it
  // for Expo instead of trying to resolve a browser-only URL at runtime.
  if (!runtimeGlobal.pdfjsWorker) {
    try {
      runtimeGlobal.pdfjsWorker = await import('pdfjs-dist/legacy/build/pdf.worker.mjs');
    } catch {
      // Native runtimes can still use the configured fake-worker source.
    }
  }
